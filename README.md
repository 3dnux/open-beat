...
- Controles: `A/D` mover, `Espacio` saltar, `J/X` atacar (combo de 3 golpes; en el aire, estocada descendente), `K/Shift` rodar (invulnerable; atravesar un ataque da una *esquiva perfecta* con cámara lenta), `Q` frasco curativo, `Enter` empezar/reintentar. En móvil aparecen botones táctiles.
- El jefe telegrafía sus ataques con un destello: garra, mordisco, barrido de cola, salto con onda de choque, aliento de fuego a ras de suelo, lluvia de meteoros y, en su fase de furia (50 % de vida), una embestida. Encadena combos, retrocede de un salto si lo acosas y te sigue con la mirada. Llenar la barra dorada lo aturde y recibe daño extra.
